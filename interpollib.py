...
def differ(x, y): #вычисление коэффициента разности
    ...
    while counter< input1- 1:
        ...
        while decount> counter:
            if counter== 0:
                ratio[decount]= ((y[decount]- y[decount-1])/ (x[decount]- x[decount- 1])) #формула
            else:
...
def function(data): #сама функци. не получается ее автоматизировать, поэтому значения подаются 
                    #вот в таком ключе
...
def generator():
    """Special interface function"""
    print('Выберите действие: сгенерировать файл (1), закинуть свой (2)')
    chose= int(input())
    if chose== 1:

        input1= int(input("Введите количество строк: "))
        main_matrix= []
    ...
    ax.plot(x,y,'o',slayx,slayy, color= 'r', mec= 'blue')
    ax= fig.add_subplot(422)
    for my, sig in space:
        ax.plot(x_values, Gauss(x_values, my, sig))
